...
class Safety :
    ...
    def chain(self):
        ...
        if weight<500:
            ...
            print('필요 줄걸이 용량(1줄당):',round(sling_capacity,2),'톤','필요 샤클 용량(1개당):',round(shackle_capacity,2),'톤')
            print(result4)
        else:
            result4='작업 불가능'
    ...
    def nylon_sling(self):
        ...
        if weight < 200:
            result4='작업 가능'
            print('총 양중 무게:',weight,'톤')
            print('필요 줄걸이 용량(1줄당):','톤',round(sling_capacity,2),'필요 샤클 용량(1개당):',round(shackle_capacity,2),'톤')
            print(result4)
        else:
            result4='작업 불가능'
...
while True:
    try:
        type1 = input('줄걸이 종류를 입력 하세요: [1.chain,2.nylon_sling,3.wire_rope]:')
        weight1 = float(input('중량물 무게를 입력 하세요 (단위:톤):'))
        weight2 = float(input('양중함 무게를 입력 하세요 (단위:톤):'))
        weight3 = float(input('줄걸이 용구 무게를 입력 하세요 (단위:톤).:'))
        num = int(input('줄걸이 수를 입력 하세요 [1,2,3,4 중 선택]:'))
